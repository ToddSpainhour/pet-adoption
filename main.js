...
const buildPetCards = (monkeyButtArray) => {
  let domString = "";
    for(i = 0; i < monkeyButtArray.length; i++) {
        ...
        domString +=    `<p class="pet-name">${monkeyButtArray[i].name}</p>`;
        domString +=      `<img src=${monkeyButtArray[i].imageUrl} class="pet-image">`;
        domString +=        `<p class="pet-color">${monkeyButtArray[i].color}</p>`;
        domString +=      `<p class="pet-skill">${monkeyButtArray[i].specialSkill}</p>`;
        domString +=    `<p id="${monkeyButtArray[i].type}">${monkeyButtArray[i].type}</p>`;
        domString += '</div>';
    } 
...
/*the reason I struggled with this excercise is because 
I was defining the argument (pets array) directly into the printToDom function 
instead of using a parameter. This was preventing it from reprinting the new array (myPets) to the DOM */
