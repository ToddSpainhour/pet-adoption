...
const buildPetCards = (array) => {
  let domString = "";
    for(i = 0; i < array.length; i++) {
        ...
        domString += `<p class="pet-name">${array[i].name}</p>`;
        domString += `<img src=${array[i].imageUrl} class="pet-image">`;
        domString += `<p class="pet-color">${array[i].color}</p>`;
        domString += `<p class="pet-skill">${array[i].specialSkill}</p>`;
        domString += `<p id="${array[i].type}">${array[i].type}</p>`;
        domString += '</div>';
    } 
